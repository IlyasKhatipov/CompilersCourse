...
struct CtorDecl : Node {
    std::vector<Param*> params;
    Stmt* body;
    CtorDecl() : body(nullptr) {}
    ~CtorDecl() {
        for (auto* p : params) delete p;
        delete body;
    }
...
struct ClassDecl : Node {
    ...
    explicit ClassDecl(std::string n) : name(std::move(n)), baseName() {}
    ~ClassDecl() {
        for (auto* v : fields) delete v;
        for (auto* c : ctors) delete c;
        for (auto* m : methods) delete m;
    }
    void print(std::ostream& os, int indent) const override {
        doIndent(os, indent);
        os << "Class: " << name;
        if (!baseName.empty()) os << " extends " << baseName;
        os << "\n";
        for (auto* v : fields) v->print(os, indent + 1);
        for (auto* c : ctors) c->print(os, indent + 1);
